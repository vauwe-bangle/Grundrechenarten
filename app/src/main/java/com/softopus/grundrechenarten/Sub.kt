...
fun SubScreen() {
    ...
    // Einstellungen für die Subtraktion laden

    val subSettings = remember {
        MathSettings(
            numberOfTasks = sharedPreferences.getInt("sub_numberOfTasks", 10),
            number1Min = sharedPreferences.getInt("sub_number1Min", 2),
            number1Max = sharedPreferences.getInt("sub_number1Max", 9),
            number2Min = sharedPreferences.getInt("sub_number2Min", 2),
            number2Max = sharedPreferences.getInt("sub_number2Max", 9)
        )
    }
    ...
    LaunchedEffect(currentTask) {
        number1 = Random.nextInt(subSettings.number1Min, subSettings.number1Max + 1)
        number2 = Random.nextInt(subSettings.number2Min, subSettings.number2Max + 1)

        // Sicherstellen, dass number1 immer größer oder gleich number2 ist
        if (number1 < number2) {
            val temp = number1
            number1 = number2
            number2 = temp
    ...
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
            .background(Color(0xFFECB3CD)), // Helles Lila
        ...
        Text(
            text = "$currentTask/${subSettings.numberOfTasks}",
        ...
        Button(
            onClick = {
                ...
                if (currentTask < subSettings.numberOfTasks) {
                    currentTask++
                    userAnswer = ""
                } else {
        ...
        if (showTable) {
            ExerciseResultsTableSub(exerciseResults = exerciseResults)
    ...
    if (showDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = {
                    saveResultsToFileSub(context, exerciseResults, elapsedTime)
                    showDialog = false
                }) {
                    Text("Ja")
                }
...
@Composable
fun ExerciseResultsTableSub(exerciseResults: List<ExerciseResult>) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 16.dp)
...
fun saveResultsToFileSub(context: Context, exerciseResults: List<ExerciseResult>, elapsedTime: Long) {
    val dateFormat = SimpleDateFormat("yyyy-MM-dd-HH-mm-ss", Locale.getDefault())
    val timestamp = dateFormat.format(Date())
    val fileName = "SUB-$timestamp.csv"
