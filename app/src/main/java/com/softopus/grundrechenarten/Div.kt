...
fun DivScreen() {
    ...
    // Einstellungen für die DIVISION laden

    val divSettings = remember {
        MathSettings(
            numberOfTasks = sharedPreferences.getInt("div_numberOfTasks", 10),
            number1Min = sharedPreferences.getInt("div_number1Min", 2),
            number1Max = sharedPreferences.getInt("div_number1Max", 9),
            number2Min = sharedPreferences.getInt("div_number2Min", 2),
            number2Max = sharedPreferences.getInt("div_number2Max", 9)
        )
    }
    ...
    LaunchedEffect(currentTask) {
        number2 = Random.nextInt(divSettings.number2Min, divSettings.number2Max + 1)
        val tempNumber1 = Random.nextInt(divSettings.number1Min, divSettings.number1Max + 1)

        // Sicherstellen, dass number1 durch number2 teilbar ist
        number1 = tempNumber1 * number2
    }
    ...
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
            .background(Color(0xFFA3EE87)), // Helles Grün
        ...
        Text(
            text = "$currentTask/${divSettings.numberOfTasks}",
        ...
        Button(
            onClick = {
                ...
                if (currentTask < divSettings.numberOfTasks) {
                    currentTask++
                    userAnswer = ""
                } else {
        ...
        if (showTable) {
            ExerciseResultsTableDiv(exerciseResults = exerciseResults)
    ...
    if (showDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = {
                    saveResultsToFileDiv(context, exerciseResults, elapsedTime)
                    showDialog = false
                }) {
                    Text("Ja")
                }
...
@Composable
fun ExerciseResultsTableDiv(exerciseResults: List<ExerciseResult>) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 16.dp)
...
fun saveResultsToFileDiv(context: Context, exerciseResults: List<ExerciseResult>, elapsedTime: Long) {
    val dateFormat = SimpleDateFormat("yyyy-MM-dd-HH-mm-ss", Locale.getDefault())
    val timestamp = dateFormat.format(Date())
    val fileName = "DIV-$timestamp.csv"
