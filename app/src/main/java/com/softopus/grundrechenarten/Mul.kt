...
fun MulScreen() {
    ...
    // Einstellungen für die MULTIPLIKATION laden

    val mulSettings = remember {
        MathSettings(
            numberOfTasks = sharedPreferences.getInt("mul_numberOfTasks", 10),
            number1Min = sharedPreferences.getInt("mul_number1Min", 2),
            number1Max = sharedPreferences.getInt("mul_number1Max", 9),
            number2Min = sharedPreferences.getInt("mul_number2Min", 2),
            number2Max = sharedPreferences.getInt("mul_number2Max", 9)
        )
    }
    ...
    LaunchedEffect(currentTask) {
        number1 = Random.nextInt(mulSettings.number1Min, mulSettings.number1Max + 1)
        number2 = Random.nextInt(mulSettings.number2Min, mulSettings.number2Max + 1)
    ...
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp)
            .background(Color(0xFFF8E967)), // Helles Gelb
        ...
        Text(
            text = "$currentTask/${mulSettings.numberOfTasks}",
        ...
        Button(
            onClick = {
                ...
                if (currentTask < mulSettings.numberOfTasks) {
                    currentTask++
                    userAnswer = ""
                } else {
        ...
        if (showTable) {
            ExerciseResultsTableMul(exerciseResults = exerciseResults)
    ...
    if (showDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = {
                    saveResultsToFileMul(context, exerciseResults, elapsedTime)
                    showDialog = false
                }) {
                    Text("Ja")
                }
...
@Composable
fun ExerciseResultsTableMul(exerciseResults: List<ExerciseResult>) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 16.dp)
...
fun saveResultsToFileMul(context: Context, exerciseResults: List<ExerciseResult>, elapsedTime: Long) {
    val dateFormat = SimpleDateFormat("yyyy-MM-dd-HH-mm-ss", Locale.getDefault())
    val timestamp = dateFormat.format(Date())
    val fileName = "MUL-$timestamp.csv"
